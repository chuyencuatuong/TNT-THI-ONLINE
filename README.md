...
- `src/lib/scoring.ts` — toàn bộ barem chấm điểm (chuẩn THPT + tuỳ chỉnh). Hàm `scoreQuestionWithAnswer` chấm 1 câu bất kỳ, được dùng CHUNG cho cả lúc nộp bài (`submitAttempt`) lẫn lúc giáo viên chấm lại sau khi sửa đáp án (`regradeAttempt`) — 2 luồng không thể trôi dạt lệch barem nhau, 42 unit test.
